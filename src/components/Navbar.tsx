      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={toggleTheme} className="p-2 rounded-lg text-foreground hover:bg-accent/10 transition-colors duration-200" aria-label="Toggle theme">
              {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>

            <div className="hidden sm:flex bg-muted rounded-full p-0.5 text-xs font-semibold transition-colors duration-300">
              <button onClick={() => setLang('en')} className={`px-2.5 py-1 rounded-full transition-colors duration-200 ${lang === 'en' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'}`}>EN</button>
              <button onClick={() => setLang('np')} className={`px-2.5 py-1 rounded-full transition-colors duration-200 ${lang === 'np' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'}`}>NP</button>
            </div>

            <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener noreferrer" className="hidden sm:block p-2 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-colors duration-200">
        ...
        {mobileOpen && (
          <div className="lg:hidden pb-4 border-t border-border transition-colors duration-300">
            {navLinks.map((link) => (
              <Link key={link.to} to={link.to} className="block px-4 py-3 text-sm font-medium text-foreground hover:text-accent hover:bg-accent/5 rounded-lg transition-colors duration-200" onClick={() => setMobileOpen(false)}>
                {link.label}
              </Link>
            ))}
            <Link to="/faq" className="block px-4 py-3 text-sm font-medium text-foreground hover:text-accent transition-colors duration-200" onClick={() => setMobileOpen(false)}>{t('faq')}</Link>
            <Link to="/track-order" className="block px-4 py-3 text-sm font-medium text-foreground hover:text-accent transition-colors duration-200" onClick={() => setMobileOpen(false)}>{t('trackOrder')}</Link>
            <div className="px-4 pt-3 flex items-center gap-3 border-t border-border mt-2">
              ...
              <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener noreferrer" className="p-2 text-green-600">
                <MessageCircle className="w-5 h-5" />
              </a>
            </div>
          </div>
