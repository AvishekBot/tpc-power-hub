    ...
    <div className="bg-background min-h-screen">
      ...
      <div className="bg-muted border-b">
        <div className="container mx-auto px-4 py-3 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Link to="/products" className="hover:text-accent">{t('products')}</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-8">
          {/* Image */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-lg aspect-square flex items-center justify-center border">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-foreground mt-3">{product.name}</h1>
            ...
            <div className={`mt-3 inline-block text-xs font-bold px-2 py-1 rounded ${inStock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {inStock ? `In Stock (${product.stock})` : t('outOfStock')}
            </div>

            <div className="mt-4 flex items-baseline gap-3">
              <span className="text-3xl font-extrabold text-accent">Rs. {(product.sale_price || product.price).toLocaleString()}</span>
              {product.sale_price && <span className="text-lg text-muted-foreground line-through">Rs. {product.price.toLocaleString()}</span>}
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Button
                disabled={!inStock}
                className="bg-amber hover:bg-amber/90 text-amber-foreground rounded-lg px-8 h-12 font-bold gap-2"
              ...
              <Button disabled={!inStock} className="bg-accent hover:bg-accent/90 text-accent-foreground rounded-lg px-8 h-12 font-bold" asChild>
                <Link to="/checkout">Buy Now</Link>
              </Button>
              <Button variant="outline" className="border-green-500 text-green-600 rounded-lg h-12 gap-2" asChild>
            ...
            <div className="border rounded-lg overflow-hidden">
              {Object.entries(product.specifications).map(([key, value], i) => (
                <div key={key} className={`flex ${i % 2 === 0 ? 'bg-muted/50' : 'bg-card'}`}>
                  <div className="w-1/3 p-3 font-semibold text-sm border-r">{key}</div>
        ...
        {related.length > 0 && (
          <div className="mt-16">
            <h2 className="text-xl font-extrabold mb-6">Related Products</h2>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {related.map((p) => (
                <Link key={p.id} to={`/products/${p.slug}`} className="bg-card rounded-lg p-4 border hover:border-accent transition-all">
                  <div className="h-32 bg-muted rounded flex items-center justify-center mb-3"><Zap className="w-10 h-10 text-muted-foreground/30" /></div>
